...
The target branch can optionally be provided as first argument, here its value: $1
When that's not present, default to `main` branch.

A GitLab issue reference can optionally be provided as second argument, here its value: $2
...
2. Look at the commits in the current branch that sit on top of the target branch.
...
5. Figure out proper title and description for a GitLab Merge Request: title must be concise and meaningful, description must still be concise but also explanatory and structured. Use GitLab-flavored markdown to the purpose. Include reference to issue "$2" (if any) at the very beginning of description; use "Related to" rather than "Closes".
